...
void rss_item_print (xmlNode *item_node, int write_pubDate, int write_link, int write_autor, int *write_new_line) {
    ...
    if (title != NULL) {
        ...
        if (*write_new_line && (write_pubDate || write_autor || write_link)) fprintf(OUT, "\n");

        fprintf(OUT, "%s\n", xmlNodeGetContent(title));
        if (pub_date != NULL && write_pubDate) fprintf(OUT, "Aktualizace: %s\n", xmlNodeGetContent(pub_date));
        if (author != NULL && write_autor) fprintf(OUT, "Autor: %s\n", xmlNodeGetContent(author));;
        if (link != NULL && write_link) fprintf(OUT, "URL: %s\n", xmlNodeGetContent(link));;
